...
def subgraph_sampler(seed, batch_nodes, samp_num_list, num_nodes, lap_matrix, labels_full, orders, device_id_of_nodes, idx_of_nodes_on_device, scale_factor,  rank, devices):
...
def ladies_sampler(seed, batch_nodes, samp_num_list, num_nodes, lap_matrix, labels_full, orders, device_id_of_nodes, idx_of_nodes_on_device, scale_factor,  rank, devices):
    '''
...
iter_num = 0
def prepare_data(pool, sampler, target_nodes, samp_num_list, num_nodes, lap_matrix, labels_full, orders, batch_size, rank, world_size, device_id_of_nodes, idx_of_nodes_on_device, devices,  scale_factor=1, local_shuffle=False, mode='train'):
    global iter_num
    if mode == 'train':
        ...
        for i in range(0, num_batches, 32):   # 32 is the queue size
            ...
            for j in range(i, min(32+i, num_batches)):
                ...
                futures.append(pool.submit(sampler, np.random.randint(2**32 - 1), target_nodes_chunk, samp_num_list, num_nodes, lap_matrix, labels_full, orders, device_id_of_nodes, idx_of_nodes_on_device, scale_factor,  rank, devices))
            yield from futures
    elif mode == 'val':
        futures = []
        ...
        futures.append(pool.submit(sampler, np.random.randint(2**32 - 1), batch_nodes, samp_num_list, num_nodes, lap_matrix, labels_full, orders, device_id_of_nodes, idx_of_nodes_on_device, [],  rank, devices))
        yield from futures
    elif mode == 'test':
        ...
        for i in range(0, num_batches, 32):   # 32 is the queue size
            ...
            for j in range(i, min(32+i, num_batches)):
                ...
                futures.append(pool.submit(sampler, np.random.randint(2**32 - 1), target_nodes_chunk, samp_num_list, num_nodes, lap_matrix, labels_full, orders, device_id_of_nodes, idx_of_nodes_on_device,  [],  rank, devices))
            yield from futures
