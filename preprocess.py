...
iter_num = 0
def prepare_data(pool, sampler, target_nodes, samp_num_list, num_nodes, lap_matrix, orders, batch_size, rank, world_size, scale_factor=1, mode='train'):
    global iter_num
    if mode == 'train' or mode == 'test':
        ...
        for i in range(0, num_batches, 32):   # 32 is the queue size
            ...
            for j in range(i, min(32+i, num_batches)):
                futures.append(pool.submit(sampler, np.random.randint(2**32 - 1), target_nodes[idxs[chunk_start+j*batch_size: min(chunk_start+(j+1)*batch_size, chunk_end)]], samp_num_list, num_nodes, lap_matrix, orders, scale_factor))
            yield from futures
    elif mode == 'val':
        futures = []
        ...
        futures.append(pool.submit(sampler, np.random.randint(2**32 - 1), batch_nodes, samp_num_list, num_nodes, lap_matrix, orders, 1))
        yield from futures
