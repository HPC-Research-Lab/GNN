...
def train(rank, devices, world_size, train_data, buffer):
    ...
    for oiter in range(1):
        ...
        susage  = GNNNodePred(encoder = encoder, num_classes=num_classes, dropout=0.1, inp = feat_data.shape[1])
        susage.to(device)
        ...
        for epoch in np.arange(args.epoch_num):
            ...
            for fut in as_completed(train_data):
                ...
                loss_train = node_pred_loss(output, torch.from_numpy(labels_full[output_nodes].todense()).to(device), args.sigmoid_loss, device)
                loss_train.backward()
            ...
            if rank == 0:
                ...
                for fut in as_completed(val_data):    
                    ...
                    loss_valid = node_pred_loss(output, torch.from_numpy(labels_full[output_nodes].todense()).to(device), args.sigmoid_loss, device).detach().tolist()
